 ...
 */
public class StateObserverSim_OLD extends ObserverBase implements StateObservation {
	//rewards
...
//	}
	...
	StateObserverSim_OLD(StateObserverSim_OLD other)
	{
...
//		setAvailableActions();		// this as replacement for availableActions.clone() would be a bit slower
	...
	@Override
	public StateObserverSim_OLD copy() 
	{
		StateObserverSim_OLD sos = new StateObserverSim_OLD(this);		// includes via 'super(other)' the copying of stored*-members in ObserverBase
...
//		if (this.storedValues!=null) sos.storedValues = this.storedValues.clone();
	...
	private void setupNodes(int numberOfNodes)
	{
		nodes = new Node[numberOfNodes];
		for(int i = 0; i < nodes.length; i++)
			nodes[i] = new Node(numberOfNodes, i+1);
	}
...
//		int dummy=1;
	 ...
	 * See triangular table in {@link StateObserverSim_OLD} for an example.
	...
	public String stringDescr2() {
		...
		for(int i = 0; i < nodes.length -1 ; i++) {
			for(int j = 0; j < nodes.length - 1; j++) {
				if (i==j && i!=nodes.length -1) {
					sout = sout + " ";
					snod = snod + " ";
				} 
				sout = sout + (str[nodes[i].getLinkPlayerPos(j)]);
				snod = snod + nodes[i].getLinkNodePos(j); 	
			}
			sout = sout + "\n";
			snod = snod + "\n";
		}
...
//	}
	...
	public int getLinkFromTo(int i, int j) {
		return nodes[i].getLinkPlayerPos(j);
	}
...
//	}
	...
	public static void main(String[] args) {
		...
		for (int i=0; i<R; i++) {
			StateObserverSim_OLD sob = (StateObserverSim_OLD) ar.getGameBoard().getDefaultStartState();
