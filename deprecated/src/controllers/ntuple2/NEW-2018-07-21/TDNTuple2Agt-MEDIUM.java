...
//		}
        ...
        for(i = 0; i < acts.size(); ++i)
        {
            if (VER_3P && UPDATE==UpdateType.SINGLE_UPDATE) {
                ...
                CurrentScoreZ = zvalueS.calculate(so,acts.get(i),refer,silent);   
                if (!randomSelect && CurrentScore!=CurrentScoreZ) {
                	int dummy=1;
                }
...
//                sanityCheck1P(so,acts,i,refer,silent,CurrentScore);
                ...
                CurrentScoreZ = zvalueM.calculate(so,acts.get(i),refer,silent);
                if (!randomSelect && CurrentScore!=CurrentScoreZ) {
                	int dummy=1;
                }
           }
