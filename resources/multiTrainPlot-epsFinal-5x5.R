...
filenames=c("multiTrain-25-6-eps000.csv","multiTrain-25-6-eps-05.csv"
           ,"multiTrain-25-6-eps002.csv")#,"multiTrain-25-6-lam060.csv"
           #,"multiTrain-25-6-lam080.csv")#,"multiTrain-25-6-lam090.csv"
           #)
# other pars: alpha=0.2, eps = 1.0, lamdbda=0.0, ChooseStart01=T
...
for (k in 1:length(filenames)) {
  ...
  #browser()
  dfBoth <- rbind(dfBoth,cbind(df,epsFin=epsilCol))
...
tgc1 <- summarySE(dfBoth, measurevar="evalQ", groupvars=c("gameNum","epsFin"))
tgc1 <- cbind(tgc1,evalMode=rep(0,nrow(tgc1)))
names(tgc1)[4] <- "eval"  # rename "evalQ"
tgc2 <- summarySE(dfBoth, measurevar="evalT", groupvars=c("gameNum","epsFin"))
tgc2 <- cbind(tgc2,evalMode=rep(10,nrow(tgc2)))
names(tgc2)[4] <- "eval"  # rename "evalT"
tgc <- rbind(tgc1,tgc2)
#tgc <- tgc1
tgc$epsFin <- as.factor(tgc$epsFin)
...
q <- ggplot(tgc,aes(x=gameNum,y=eval,colour=epsFin,linetype=evalMode))
