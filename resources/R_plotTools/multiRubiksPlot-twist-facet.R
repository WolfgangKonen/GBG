#
# **** These are new results with MCTSWrapper[TCL4-EXP] from Dec-2021/Jan-2022 ****
#
...
# 
# This is the more condensed facet plot, see multiRubiksPlot-twist.R for single-plot variant.
# errWidth and lineSize below are optimized for the PDF plot appearance.
#
library(ggplot2)
library(grid)
...
pdffile=paste0("Rubiks-both-cubes-ptwist-facet.pdf")
...
tgc <- summarySE(dfBoth, measurevar="winrate", groupvars=c("iterMWrap","cubeWidth","pTwist","twistType"))
...
#q <- q+geom_line(position=pd,size=lineSize) + geom_point(position=pd,size=2.0) 
q <- q+geom_line(size=lineSize) + geom_point(size=2.0)
