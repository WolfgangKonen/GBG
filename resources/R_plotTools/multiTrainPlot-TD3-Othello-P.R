...
filenames=c("multiTrain-lam05_P.csv"
           #,"multiTrain_TCL-EXP-NT3-al50-lam016-500k-HOR001-T-epsfin0.csv"
           )
# other pars: eps = 0.2->0.2, gamma = 1.0, ChooseStart01=F, NORMALIZE=F, SIGMOID=tanh, 
# LEARNFROMRM=F, random ntuple mode 1: 50 6-tuples. TC_INIT=1e-4, TC_EXP
# with TC beta =2.7, rec.weight-change accumulation. 200.000 training games, 3 runs.
# evalMode= 10 (evalQ) is from default start state against HeurPlayer, 
# 
...
tgc1 <- summarySE(dfBoth, measurevar="evalQ", groupvars=c(gamesVar,"P"))
tgc1 <- cbind(tgc1)
names(tgc1)[4] <- "eval"  # rename "evalQ"
tgc2 <- summarySE(dfBoth, measurevar="evalT", groupvars=c(gamesVar,"P"))
tgc2 <- cbind(tgc2)
names(tgc2)[4] <- "eval"  # rename "evalT"
tgc <- rbind(tgc1,tgc2)
tgc <- tgc2
tgc$P <- as.factor(tgc$P)
...
if (USEGAMESK) {
  q <- ggplot(tgc,aes(x=gamesK,y=eval,shape=P,color=P))
  q <- q + xlab(bquote(paste("games [*",10^3,"]", sep=""))) + ylab(evalStr)
} else {
  q <- ggplot(tgc,aes(x=gameNum,y=eval,colour=P,linetype=P))
}
q <- q+geom_errorbar(aes(ymin=eval-se, ymax=eval+se), width=1*errWidth, position=pd)
