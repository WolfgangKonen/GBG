...
MAPWINRATE=F      # if =T: map y-axis to win rate (range [0,1]); if =F: range [-1,1]
...
evalStr = ifelse(MAPWINRATE,"win rate", "avg reward")
path <- "../../agents/Nim3P/N03-S05-ExtraYes/csv/"; 
Ylimits=c(ifelse(MAPWINRATE,0.0,0.0),1.0); errWidth=1000/wfac;
...
filenames=c(#"multiTrain-OLD.csv"  # GBGBatch-OLD.jar,branch master as of 2020-03-10
            # "multiTest-FA.csv"      # GBGBatch.jar
            #,"multiTest-noFA.csv"    # GBGBatch-noFA.jar
             "multiTest-lam05-RESET-300k-FA.csv"      # GBGBatch.jar
            ,"multiTest-lam05-RESET-300k-noFA.csv"    # GBGBatch-noFA.jar
)
...
tgc2 <- cbind(tgc2,evalMode=rep("MCTS",nrow(tgc2)))
names(tgc2)[5] <- "eval"  # rename "evalT"
tgc <- rbind(tgc1,tgc2) # MaxN & MCTS
#tgc <- tgc2              # MCTS only
...
if (USEGAMESK) {
  q <- ggplot(tgc,aes(x=gamesK,y=eval,shape=evalMode,linetype=evalMode,color=evalMode))
  q <- q + xlab(bquote(paste("episodes [*",10^3,"]", sep=""))) + ylab(evalStr)
  q <- q+scale_x_continuous(limits=Xlimits) 
} else {
  q <- ggplot(tgc,aes(x=gameNum,y=eval,colour=algo,linetype=evalMode))
}
