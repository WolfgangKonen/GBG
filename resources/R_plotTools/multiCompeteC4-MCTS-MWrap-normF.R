...
tgc <- summarySE(dfBoth, measurevar="winrate", groupvars=c("iterMWrap","EPS","opponent"))
...
q <- ggplot(tgc,aes(x=iterMWrap,y=winrate,colour=opponent,shape=opponent))
q <- q+labs(title="MCTS vs. opponent")
...
plot(q)
pdffile="C4-MCTS-vs-Opponent.pdf"
