#
# **** These are new results with MCTSWrapper[TCL4-EXP], nSym, and QTM from Jan 2023 ****
#      They should be statistically the same as those from multiRubiksPlot-nSym-beyond.R
#
# **** see myDocs/Papers-with-Code-Rubiks.docx (and notes-WK-RubiksCube, search for 'beyond').
#      Results for 3x3x3,QTM are read from symmIterSingle-batch.csv, created with
#      experiments/RubiksCube/experim_6-4.sh.
# 
library(ggplot2)
library(grid)
...
TWISTTYPE="QTM"
pdffile=paste0("Rubiks-nsym-ptwist-",TWISTTYPE,"-ET16-batch.pdf")
...
for (CUBEW in c(3)) {        # cube width, either 2 or 3, currently only 3
  ...
  if (TWISTTYPE=="HTM") {
    ...
    fname <- switch(CUBEW
                    ...
                    ,paste0("symmIterSingle-batch.csv")   # CUBEW=3, QTM, cPUCT=1
