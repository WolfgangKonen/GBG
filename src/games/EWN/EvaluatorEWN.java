...
//  }
        ...
        if (diffStarts) 	// start from all start states in diffStartList
        {
            ...
            for (int c=0; c<numEpisodes; c++) {
                for (StateObservation sd : diffStartList) {
                    sc = XArenaFuncs.competeNPlayerAllRoles(new PlayAgtVector(playAgent, opponent), sd, 100, 0, null, null, false, null);
    ...
    private EvalResult evalAgainstTwoOpponents(PlayAgent playAgent, PlayAgent opponent, PlayAgent opponent2,
        ...
        if (diffStarts) 	// start from all start states in diffStartList
        {
            ...
            for (int c=0; c<numEpisodes; c++) {
                for (StateObservation sd : diffStartList) {
                    sc = XArenaFuncs.competeNPlayerAllRoles(new PlayAgtVector(playAgent, opponent, opponent2), sd, 100, 0, null, null, false, null);
                    scMean.combine(sc, ScoreTuple.CombineOP.AVG, 0, sWeight);
                    //count++;
                }
            }
        }else {
            scMean= XArenaFuncs.competeNPlayerAllRoles(new PlayAgtVector(playAgent,opponent, opponent2), so, numEpisodes,0, null, null, false, null);
        }
        lastResult = scMean.scTup[0];
        m_msg = playAgent.getName() + ": " + getPrintString() + lastResult;
        return new EvalResult(lastResult, lastResult>thresh, m_msg, m_mode, thresh);
    ...
    private EvalResult evalAgainstThreeOpponents(PlayAgent playAgent, PlayAgent opponent, PlayAgent opponent2,PlayAgent opponent3,
        ...
        if (diffStarts) 	// start from all start states in diffStartList
        {
            ...
            for (int c=0; c<numEpisodes; c++) {
                for (StateObservation sd : diffStartList) {
                    sc = XArenaFuncs.competeNPlayerAllRoles(new PlayAgtVector(new PlayAgent[]{playAgent,opponent, opponent2, opponent3}), sd, 100, 0, null, null, false, null);
                    scMean.combine(sc, ScoreTuple.CombineOP.AVG, 0, sWeight);
                    //count++;
                }
            }
        }else {
            scMean= XArenaFuncs.competeNPlayerAllRoles(new PlayAgtVector(new PlayAgent[]{playAgent,opponent, opponent2, opponent3}), so, numEpisodes,0, null, null, false, null);
        }
        lastResult = scMean.scTup[0];
        m_msg = playAgent.getName() + ": " + getPrintString() + lastResult;
        return new EvalResult(lastResult, lastResult>thresh, m_msg, m_mode, thresh);
