...
//	private void initEvaluator(GameBoard gb) { 	}
	 ...
	 */
 	private EvalResult evaluateAgent1(PlayAgent pa, PlayAgent opponent, GameBoard gb, double thresh) {
		...
		ScoreTuple sc = XArenaFuncs.competeNPlayerAllRoles(new PlayAgtVector(pa,opponent), so, 1, 0, null, null, false, null);
...
//		lastResult=0;
        ...
        for (StateObserverTTT so : diffStartList) {
    		ScoreTuple sc = XArenaFuncs.competeNPlayerAllRoles(new PlayAgtVector(pa, opponent), so, competeNum, 0, null, null, false, null);
    		success = sc.scTup[0];
