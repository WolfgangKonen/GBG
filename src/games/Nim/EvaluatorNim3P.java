...
//		lastResult = XArenaFuncs.competeBoth(pa, random_agent, so, 100, 0, gb);
		ScoreTuple sc = XArenaFuncs.competeNPlayerAllRoles(new PlayAgtVector(pa,randomAgent,randomAgent2), so, competeNum, 0, null, null);
	 ...
	 */
 	private EvalResult evaluateAgent1(PlayAgtVector paVector, GameBoard gb, int competeNum, double thresh) {
 		StateObservation so = gb.getDefaultStartState(null);
		ScoreTuple sc = XArenaFuncs.competeNPlayerAllRoles(paVector, so, competeNum, 0, null, null);
		lastResult = sc.scTup[0];
...
//		double resX, resO;
		...
		for (int k=0; k<numK; ++k) {
			...
			ScoreTuple sc = XArenaFuncs.competeNPlayerAllRoles(paVector, startSO, competeNum, 0, null, null);
    		success = sc.scTup[0];
