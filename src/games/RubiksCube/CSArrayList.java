...
/**
 * This is a class for generating distance sets for the cube.
 */
public class CSArrayList extends ArrayList<CubeState> {
	 ...
	 * @see TupleInt 
	 */
	public CSArrayList(CSAListType csa, CSArrayList D, CSArrayList Dprev, int N,
		...
		for (int n=0; n<N; n++) {
			...
			// which seems more natural for a distance set D), since only with an ArrayList 
...
//			System.out.println(cS0.twistSeq+"  "+n);
			...
			for (int i=1; i<=3; i++) {
				for (int act1=0; act1<3; act1++) {
					...
					if (twist[act1]!=cS0.lastTwist) {
						CubeState cS1 = new CubeState(cS0);
						switch(twist[act1]) {
						...
						case GenerateNext: 
							...
							break;
						default: 
							throw new RuntimeException("CSAListType "+csa+" not allowed!");
						}
						//
						...
						if (this.contains(cS1)) {
							twinCounter++;
							if (csa == CSAListType.GenerateNext) {
	...
	/**
	 * Assert for each CubeState in this, that its twist sequence applied to the 
	...
	/**
	 * Check if there is a twin to {@code other} in {@code this}
	 * @param other
	 ...
	 *  from  CSArrayList(GenerateNext*,...) <ul>
	 *  <li> n: the for-loop counter
	 *  <li> setSize: 	the size of the set just added
	 *  <li> dSize:		the current size of the ArrayList
	 *  <li> prevCounter	how many generated states belong to Dprev
	 *  <li> currCounter	how many generated states belong to D
	 *  <li> twinCounter	how many generated states have a twin in Dnext
	 *  </ul>
	 */
