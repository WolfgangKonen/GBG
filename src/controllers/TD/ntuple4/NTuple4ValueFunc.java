...
//		this.useSymmetry = useSymmetry;
		...
		for (int i = 0; i < numTuples; i++) {
			...
			for (int o=0; o<numOutputs; o++) {
				for (int k=0; k<numPlayers; k++) {
					this.nTuples[o][k][i] = new NTuple4(nTuplesI[i], posVals, ntPar);
					if (randInitWeights) {
	...
	public double getScoreI(StateObsWithBoardVector curSOWB, int player) {
		...
		//if (TDNTuple4Agt.NEWTARGET) score /= equiv.length;
	...
	private BoardVector[] getSymBoards2(StateObsWithBoardVector curSOWB, boolean useSymmetry, int nSym) {
		...
		if (useSymmetry) {
			...
			if (tdAgt instanceof Sarsa4Agt && nSym < xnf.getNumSymmetries())
				...
				throw new RuntimeException("[NTuple4ValueFunc] Sorry, cannot handle case SarsaAgt and 0 < nSym < s (symmetryActions not yet adapted).");
...
//			int dummy=1;
	 ...
	 * 
	 * @param curSOWB
	 *            the board for which the NN is to be adapted. This is in case of {@link TDNTuple4Agt} the
	...
	/**
	 * Weighted update of weights of the n-tuple system in case of *new* TD-learning ({@link TDNTuple4Agt}).
	 *
	 * @param curSOWB
	 *            the board for which the NN is to be adapted. This is in case of {@link TDNTuple4Agt} the
	 *            afterstate generated by curPlayer <b>one round earlier</b>, i.e. {@code sLast[curPlayer]}
	 ...
	 *            the player who generated the current board
	 * @param weight
	 *            a weight applied to the delta or loss term
	 * @param vLast
	 *            the value V(s) for s=curBoard and player curPlayer (NN estimate)
	 * @param target
	 *            the target to learn, usually (reward + GAMMA * value of the actual afterstate) for
	 ...
	 * 			  only for debug info: access to the current state's stringDescr()
	 */
	public void w_updateWeightsTD(StateObsWithBoardVector curSOWB, int curPlayer, double weight,
								double vLast, double target, double reward, StateObservation thisSO) {
		// delta is the error signal:
		double delta = weight*(target - vLast);
		// derivative of tanh ( if hasSigmoid()==true):
		double e = (hasSigmoid() ? (1.0 - vLast * vLast) : 1.0);
...
//			int dummy=1;
	 ...
	 * @param QMODE :<br>  
	 * 			  {@code true}, if called via {@code updateWeightsQ} (Q-learning via {@link Sarsa4Agt});<br>
	 * 			  {@code false}, if called from TD-learning (via  {@code updateWeightsNew*}, {@code TDNTuple2Agt},
	 * 			  or via {@code updateWeightsTD}, {@link TDNTuple4Agt})
	 * @param ELIST_PP (eList-per-player, whether to keep separate eligibility lists per player):<br>   
	 * 			  {@code true}, if called from 'new' TD-learning {@link Sarsa4Agt} or {@link TDNTuple4Agt};<br>
	 * 			  {@code false}, if called from 'old' TD-learning (via {@code TDNTuple2Agt})
	 */
	protected void update(StateObsWithBoardVector curSOWB, int player, int output, double delta, double e,
		...
		assert (horizon>0) : "[NTuple4ValueFunc.update] Error: horizon is 0 !";
...
//					System.out.print("(i,j)=("+i+","+j+"):  ");		//debug
	...
	public void setTdAgt(NTuple4Agt tdAgt) {
		this.tdAgt = tdAgt;
	}
